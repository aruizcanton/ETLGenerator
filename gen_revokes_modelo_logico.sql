...
BEGIN
  ...
  IF num_filas > 0 THEN
    ...
    LOOP
      ...
      DBMS_OUTPUT.put_line('REVOKE select, insert, update, delete, alter on app_mvnodm.' || r_mtdt_modelo_logico_TABLA.TABLE_NAME || ' from app_mvnotc;');
      DBMS_OUTPUT.put_line('REVOKE select, insert, update, delete, alter on app_mvnodm.T_' || nombre_tabla_reducido || ' from app_mvnotc;');
