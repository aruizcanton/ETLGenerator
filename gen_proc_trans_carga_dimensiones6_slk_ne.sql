...
cursor MTDT_TABLA
  is
    ...
    --and TABLE_NAME in ('SA_FACT_SERIADOS1', 'SA_MOVIMIENTOS_SERIADOS1', 'SA_PARQUE_SERIADOS1', 'SA_MOVIMIENTOS_SERIADOS')
    ...
    --and TABLE_NAME in ('DMD_SERIADO', 'DMD_ALMACEN', 'DMD_MATERIAL')
    ...
    then
            ...
            loop
              ...
              then
                ...
                /* (20220928) Angel Ruiz. Ocurre que vienen las dimensiones con Subqueries. Antes las dimensiones solo tiraban */
                /* de las tablas de Staging.*/
                v_existe_select_tbsn:=0;
                if (regexp_instr (reg_scenario.TABLE_BASE_NAME,'[Ss][Ee][Ll][Ee][Cc][Tt]') > 0) then
                  v_existe_select_tbsn:=1;
                  /* Aparecen queries en lugar de tablas en la columna de nombre de TABLE_BASE_NAME */
                  if (REGEXP_LIKE(reg_scenario.TABLE_BASE_NAME, '\) *[a-zA-Z_0-9]+$')) then
                  /* Se aceptan tablas de TABLE_BASE_NAME que son SELECT que ademas tienen un ALIAS */
                    v_alias_tbsn := trim(substr(REGEXP_SUBSTR (reg_scenario.TABLE_BASE_NAME, '\) *[a-zA-Z_0-9]+$'), 2));
                    --mitabla_look_up := reg_detalle_in.TABLE_LKUP;
                    mitabla_tbsn := procesa_campo_filter(reg_scenario.TABLE_BASE_NAME);
                    v_alias_tbsn_incluido := 1;
                    dbms_output.put_line('EXISTE ALIAS EN LA QUERY TABLE_BASE_NAME');
                  else
                    v_alias_tbsn := 'TBSN';
                    mitabla_tbsn := '(' || procesa_campo_filter(reg_scenario.TABLE_BASE_NAME) || ') "TBSN"';
                    --mitabla_look_up := '(' || reg_detalle_in.TABLE_LKUP || ') "LKUP_' || l_FROM.count || '"';
                    v_alias_tbsn_incluido := 0;
                    dbms_output.put_line('NO EXISTE ALIAS EN LA QUERY TABLE_BAE_NAME');
                  end if;
                  ...
                  if (instr(reg_scenario.TABLE_BASE_NAME, '.') > 0) then
                    UTL_FILE.put_line(fich_salida_pkg, '    ' || reg_scenario.TABLE_BASE_NAME || ', ' || OWNER_DM || '.' || reg_scenario.TABLE_NAME);
                  else
                ...
                THEN
                  ...
                  LOOP
                    ...
                    if (regexp_instr(where_table_columns(indx), '[Dd][Ee][Cc][Oo][Dd][Ee]') > 0 or regexp_instr(where_interface_columns(indx), '[Dd][Ee][Cc][Oo][Dd][Ee]') > 0) then
                      /* (20220928) Angel Ruiz. Pueden venir subqueries en TBALE_BASE_NAME */
                      if (v_existe_select_tbsn = 1) then
                        UTL_FILE.put_line(fich_salida_pkg,'    ' || transformo_decode(where_interface_columns(indx), v_alias_tbsn, 0) || ' = ' || transformo_decode(where_table_columns(indx), reg_scenario.TABLE_NAME, 1) || ' AND');
                      else
                      ...
                      if (v_existe_select_tbsn = 1) then
                        UTL_FILE.put_line(fich_salida_pkg,'    ' || transformo_funcion_outer (where_interface_columns(indx), v_alias_tbsn, 0) || ' = ' || transformo_funcion_outer (where_table_columns(indx), reg_scenario.TABLE_NAME, 1) || ' AND');
                      else
                      ...
                      /* (20220928) Angel Ruiz. Pueden venir subqueries en TBALE_BASE_NAME */
                      if (v_existe_select_tbsn = 1) then
                        /* La columna TABLE_BASE_NAME es una select */
                        UTL_FILE.put_line(fich_salida_pkg,'    ' || v_alias_tbsn || '.' || where_interface_columns(indx) || ' = ' || reg_scenario.TABLE_NAME || '.' || where_table_columns(indx) || '(+) AND');
                      else
                  ...
                  LOOP
                    IF indx = where_interface_columns.LAST THEN
                      ...
                      if (regexp_instr(where_table_columns(indx), '[Dd][Ee][Cc][Oo][Dd][Ee]') > 0 or instr(where_interface_columns(indx), '[Dd][Ee][Cc][Oo][Dd][Ee]') > 0) then
                        if (v_existe_select_tbsn = 1) then
                          UTL_FILE.put_line(fich_salida_pkg,'    ' || transformo_decode (where_interface_columns(indx), v_alias_tbsn, 0) || ' = ' || transformo_decode (where_table_columns(indx), reg_scenario.TABLE_NAME, 0));
                        else
                        ...
                        if (v_existe_select_tbsn = 1) then
                          UTL_FILE.put_line(fich_salida_pkg,'    ' || transformo_funcion_outer (where_interface_columns(indx), v_alias_tbsn, 0) || ' = ' || transformo_funcion_outer (where_table_columns(indx), reg_scenario.TABLE_NAME, 0));
                        else
                        ...
                        if (v_existe_select_tbsn = 1) then
                          UTL_FILE.put_line(fich_salida_pkg,'    ' || v_alias_tbsn || '.' || where_interface_columns(indx) || ' = ' || reg_scenario.TABLE_NAME || '.' || where_table_columns(indx));
                        else
                    ...
                    ELSE
                      ...
                      if (regexp_instr(where_table_columns(indx), '[Dd][Ee][Cc][Oo][Dd][Ee]') > 0 or instr(where_interface_columns(indx), '[Dd][Ee][Cc][Oo][Dd][Ee]') > 0) then
                        if (v_existe_select_tbsn = 1) then
                          UTL_FILE.put_line(fich_salida_pkg,'    ' || transformo_decode (where_interface_columns(indx), v_alias_tbsn, 0)  || ' = ' || transformo_decode (where_table_columns(indx), reg_scenario.TABLE_NAME, 0) || ' and');
                        else
                        ...
                        if (v_existe_select_tbsn = 1) then
                          UTL_FILE.put_line(fich_salida_pkg,'    ' || transformo_funcion_outer (where_interface_columns(indx), v_alias_tbsn, 0) || ' = ' || transformo_funcion_outer (where_table_columns(indx), reg_scenario.TABLE_NAME, 0) || ' and');
                        else
              ...
              then
                ...
                THEN
                  ...
                  LOOP
                    ...
                    if (regexp_instr(where_table_columns(indx), '[Dd][Ee][Cc][Oo][Dd][Ee]') > 0 or regexp_instr(where_interface_columns(indx), '[Dd][Ee][Cc][Oo][Dd][Ee]') > 0) then
                      if (v_existe_select_tbsn = 1) then
                        UTL_FILE.put_line(fich_salida_pkg,'    ' || transformo_decode (where_table_columns(indx), reg_scenario.TABLE_NAME, 0) || ' = ' || transformo_decode (where_interface_columns(indx), v_alias_tbsn, 1) || ' AND');                      
                      else
                      ...
                      if (v_existe_select_tbsn = 1) then
                        UTL_FILE.put_line(fich_salida_pkg,'    ' || transformo_funcion_outer (where_table_columns(indx), reg_scenario.TABLE_NAME, 0) || ' = ' || transformo_funcion_outer (where_interface_columns(indx), v_alias_tbsn, 1) || ' AND');
                      else
                      ...
                      if (v_existe_select_tbsn = 1) then
                        UTL_FILE.put_line(fich_salida_pkg,'    ' || reg_scenario.TABLE_NAME || '.' || where_table_columns(indx) || ' = ' || v_alias_tbsn || '.' || where_interface_columns(indx) || ' (+) AND');
                      else
                  ...
                  if (regexp_instr(where_interface_columns ( where_interface_columns.FIRST), '[Ll][Tt][Rr][Ii][Mm]') > 0) then
                    if (v_existe_select_tbsn = 1) then
                      UTL_FILE.put_line(fich_salida_pkg, '    '  || transformo_funcion_outer (where_interface_columns ( where_interface_columns.FIRST), v_alias_tbsn, 0) || ' IS NULL' );
                    else
                    ...
                    if (v_existe_select_tbsn = 1) then
                      UTL_FILE.put_line(fich_salida_pkg, '    ' || v_alias_tbsn || '.' || where_interface_columns(where_interface_columns.FIRST) || ' IS NULL' );
                    else
        ...
        /***********************/
            ...
            UTL_FILE.put_line(fich_salida_hist, '################################################################################');
            UTL_FILE.put_line(fich_salida_hist, '. ${' || NAME_DM || '_ENTORNO}/entorno' || NAME_DM || '_' || PAIS || '.sh');
